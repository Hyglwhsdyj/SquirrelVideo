...
//        L.d(TAG, "--------ma chunk");
    ...
    public void startDownload(int token) throws IOException {
        L.d(TAG, token + "------startDownload------");
        // switch on task state
        L.d(TAG, token + "------startDownload------before getTaskInfo");
        Task task = tasksDataSource.getTaskInfo(token);
        L.d(TAG, token + "------startDownload------after getTaskInfo");
        L.d(TAG, token + "------startDownload------");
        L.d(TAG, token + "------startDownload------before AsyncStartDownload Thread");
        Thread asyncStartDownload
                = new AsyncStartDownload(tasksDataSource, chunksDataSource, moderator, downloadManagerListener, task);
        L.d(TAG, token + "------startDownload------after AsyncStartDownload Thread");
        asyncStartDownload.start();
        L.d(TAG, token + "------startDownload------AsyncStartDownload started");
    }
    ...
    private int insertNewTask(String taskName, String url, int chunk, String save_address, boolean priority) {
        ...
        L.d(TAG, task.id + "------addTask------TaskStates.INIT");
//        L.d(TAG, "--------task id " + String.valueOf(task.id));
        return task.id;
    }
