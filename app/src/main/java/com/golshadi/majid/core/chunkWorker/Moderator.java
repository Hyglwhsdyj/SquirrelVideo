...
public class Moderator {
    ...
    private static final String TAG = Moderator.class.getSimpleName() + ":";
    ...
    public void start(Task task, DownloadManagerListenerModerator listener) {
        ...
        if (taskChunks != null) {
            ...
            L.d(TAG, task.id + "------startDownload------TaskStates.DOWNLOADING");
            // get any chunk file size calculate
            for (Chunk chunk : taskChunks) {
                L.d(TAG, task.id + "______chunk______" + chunk);
                downloaded = new Long(FileUtils.size(task.save_address, String.valueOf(chunk.id)));
                totalSize = new Long(chunk.end - chunk.begin + 1);
                L.d(TAG, task.id + "______downloaded______" + downloaded + "______totalSize______" + totalSize);
                if (!task.resumable) {
                    L.d(TAG, task.id + "______resumable______" + task.resumable);
                    chunk.begin = 0;
                    chunk.end = 0;
                    ...
                    L.d(TAG, task.id + "______chunk.begin______" + chunk.begin);

                    // start any of them as AsyncTask
                    Thread chunkDownloaderThread = new AsyncWorker(task, chunk, this);
                    workerList.put(chunk.id, chunkDownloaderThread);
                    L.d(TAG, task.id + "______workerList______" + workerList.size());
                    chunkDownloaderThread.start();
                }
            }
            // notify to developer------------------------------------------------------------
    ...
    private final int THRESHOLD = 1024 * 800;
