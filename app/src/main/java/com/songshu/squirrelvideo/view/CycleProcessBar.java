...
public class CycleProcessBar extends View {
    ...
    public CycleProcessBar(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        switch (style) {
            case STROKE: {
                ...
                canvas.drawArc(oval, 0, 360 * progress / max, false, paint);  //根据进度画圆弧 // 修改第二个参数修改画进度的起点位置
                break;
            }
            case FILL: {
                paint.setStyle(Paint.Style.FILL_AND_STROKE);
                if (progress != 0)
                    canvas.drawArc(oval, 0, 360 * progress / max, true, paint);  //根据进度画圆弧 // 修改第二个参数修改画进度的起点位置
                break;
            }
        }
