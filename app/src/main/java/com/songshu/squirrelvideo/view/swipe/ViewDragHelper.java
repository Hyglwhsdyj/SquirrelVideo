...
package com.songshu.squirrelvideo.view.swipe;
...
public class ViewDragHelper {
     ...
     * {@link com.way.swipeback.ViewDragHelper.Callback#tryCaptureView(View, int)}
     ...
     */
    private float currentDownX; // 记录当前按下的点
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                // report edge drags.
                if (ev.getX() - currentDownX < 0 ||
                        (CalculationOfAngle(currentDownX, ev.getX(), currentDownY, ev.getY()) > 30)) { // 如果是向左滑，就把事件向下分发
                    return false;
                }
                final int pointerCount = MotionEventCompat.getPointerCount(ev);
                for (int i = 0; i < pointerCount; i++) {
     ...
     * @return
     */
    private float CalculationOfAngle(float x1, float x2, float y1, float y2) {
