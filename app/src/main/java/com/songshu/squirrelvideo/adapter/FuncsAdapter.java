 ...
 */
public class FuncsAdapter extends MyBaseAdapter {

    private static final String TAG = FuncsAdapter.class.getSimpleName() + ":";
    private Context mContext;
    private List<FuncsDBDataBean> dataList;
    private boolean isDeleteIconShow = false;


    public FuncsAdapter(Context mContext) {
    ...
    public View getView(int position, View convertView, ViewGroup parent, boolean isShowDeleteOnly) {
        ...
        if (convertView == null) {
            holder = new Holder();
            convertView = View.inflate(mContext, R.layout.item_funcs, null);
        ...
        if ((position * 5) < dataList.size()) {
            ...
            FuncsDBDataBean bean = dataList.get(position * 5);
            if (!isShowDeleteOnly) {
        ...
        if ((position * 5 + 1) < dataList.size()) {
            ...
            FuncsDBDataBean bean = dataList.get(position * 5 + 1);
            if (!isShowDeleteOnly) {
        ...
        if ((position * 5 + 2) < dataList.size()) {
            ...
            FuncsDBDataBean bean = dataList.get(position * 5 + 2);
            if (!isShowDeleteOnly) {
        ...
        if ((position * 5 + 3) < dataList.size()) {
            ...
            FuncsDBDataBean bean = dataList.get(position * 5 + 3);
            if (!isShowDeleteOnly) {
        ...
        if ((position * 5 + 4) < dataList.size()) {
            ...
            FuncsDBDataBean bean = dataList.get(position * 5 + 4);
            if (!isShowDeleteOnly) {
                ...
                holder.rl_5.setOnClickListener(clickListener);
                holder.rl_5.setTag(bean);
...
//            dataList.remove(mFuncsDBDataBean);
//            notifyDataSetChanged();
